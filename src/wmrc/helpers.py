...
# @staticmethod
# def recycling_rates_per_material(year_df: pd.DataFrame, field_mapping: Mapping[str, str]) -> pd.DataFrame:
#     """Calculate the recycling rates for each material type for a given year.

#     Args:
#         year_df (pd.DataFrame): Dataframe of facility records for a single year (can be .applied to a groupby
#         (year)).
#         field_mapping (Mapping[str, str]): Mapping of field names from manual reports to Salesforce field names.
